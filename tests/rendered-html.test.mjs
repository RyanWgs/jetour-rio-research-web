...
test('bilingual routes and approved module order are present', async () => {
  ...
  assert.match(explorer, /zh:\s*\[[\s\S]+festival[\s\S]+venue[\s\S]+hotel[\s\S]+beach_case[\s\S]+partnership[\s\S]+\],/);
  assert.match(explorer, /en:\s*\[[\s\S]+festival[\s\S]+venue[\s\S]+hotel[\s\S]+beach_case[\s\S]+ip[\s\S]+communication[\s\S]+partnership[\s\S]+\],/);
  assert.match(explorer, /number: '01'/);
  assert.match(explorer, /number: '02'/);
  assert.match(explorer, /number: '03'/);
  assert.match(explorer, /number: '04'/);
  assert.match(explorer, /number: '05'/);
  assert.match(explorer, /number: '06'/);
  assert.match(explorer, /number: '07'/);
  assert.match(explorer, /酒店配套/);
  assert.match(explorer, /沙滩音乐节案例/);
  assert.match(explorer, /异业合作/);
  assert.match(explorer, /Hotel Support/);
  assert.match(explorer, /Beach Festival Cases/);
  assert.match(explorer, /Cross-industry Partnerships/);
  assert.match(explorer, /showHotelsForVenue/);
  assert.match(explorer, /linkedVenueId/);
